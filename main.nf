...
if (params.genomefai)              { genomefai_ch = Channel.fromPath(params.genomefai, checkIfExists: true) }        else { exit 1, 'Genome fai not specified!' }
if (params.blacklist)              { blacklist_ch = Channel.fromPath(params.blacklist, checkIfExists: true) }        else { exit 1, 'Black list not specified!' }
if (params.fasta)                  { fasta_ch = Channel.fromPath(params.fasta, checkIfExists: true) }                else { exit 1, 'Fasta not specified!' }
if (params.gtf)                    { gtf_ch = Channel.fromPath(params.gtf, checkIfExists: true) }                    else { exit 1, 'GTF not specified!' }
